...
export function MemoriesPage({ memories, onCapture, onSelectMemory, preferences }) {
    ...
    <div className="subpage memories-page">
      ...
      {memories.length ? (
        <section className="memory-board" aria-label="Memory library">
          ...
          {filteredMemories.length ? <div className="memory-card-grid">{filteredMemories.map((memory) => <MemoryCard memory={memory} onSelect={onSelectMemory} key={memory.id} variant={preferences?.compactCards ? "compact" : "library"} />)}</div> : <div className="memory-filter-empty"><MagnifyingGlass /><p>No {activeType} memories yet.</p><button type="button" onClick={onCapture}>Capture one</button></div>}
        </section>
